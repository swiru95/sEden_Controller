...
EXECUTE_ON_STARTUP(
    omnetpp::cEnum *e = omnetpp::cEnum::find("kind");
    if (!e) omnetpp::enums.getInstance()->add(e = new omnetpp::cEnum("kind"));
    e->insert(ECHO_INTERVAL_TIMER, "ECHO_INTERVAL_TIMER");
    e->insert(ECHO_CANCEL_TIMER, "ECHO_CANCEL_TIMER");
    e->insert(PORT_BLOCK_TIMER, "PORT_BLOCK_TIMER");
    e->insert(FEATURE_WAIT_TIMER, "FEATURE_WAIT_TIMER");
    e->insert(HELLO_WAIT_TIMER, "HELLO_WAIT_TIMER");
)

Register_Class(timer)

timer::timer(const char *name, short kind) : ::omnetpp::cMessage(name, kind)
{
}

timer::timer(const timer& other) : ::omnetpp::cMessage(other)
{
    copy(other);
}
...
uint8_t timer::getType() const
{
    return this->type;
}

void timer::setType(uint8_t type)
{
    this->type = type;
}

int timer::getConnId() const
{
    return this->connId;
}

void timer::setConnId(int connId)
{
    this->connId = connId;
}
...
timerDescriptor::timerDescriptor() : omnetpp::cClassDescriptor(omnetpp::opp_typename(typeid(timer)), "omnetpp::cMessage")
{
...
int timerDescriptor::findField(const char *fieldName) const
{
    ...
    if (fieldName[0] == 'c' && strcmp(fieldName, "connId") == 0) return base+1;
...
bool timerDescriptor::setFieldValueAsString(void *object, int field, int i, const char *value) const
{
    ...
    switch (field) {
        ...
        case FIELD_connId: pp->setConnId(string2long(value)); return true;
